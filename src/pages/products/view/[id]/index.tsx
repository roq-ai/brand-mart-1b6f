...
import { deleteCartProductById } from 'apiSdk/cart-products';
...
function ProductViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProductInterface>(
    ...
    () =>
      getProductById(id, {
        relations: ['company', 'cart_product'],
      }),
  );

  const cart_productHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('cart_product', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Cart Products
                    </Text>
                    <NextLink passHref href={`/cart-products/create?product_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.cart_product?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/cart-products/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('cart_product', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/cart-products/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('cart_product', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    cart_productHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
